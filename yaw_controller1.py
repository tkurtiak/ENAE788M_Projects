...
def solver(msg):
    ...
    if 100 < abs(msg.linear.x) < 4000 and 100 < abs(msg.linear.y) < 4000 and 100 < abs(msg.linear.z) < 3000 and abs(msg.angular.z) < np.pi*.6:
        ...
        # if error>100:
        #     velocity.angular.z = -1*yaw*error*(np.pi/180)
        # else:
        #     velocity.angular.z = 0
        velocity.angular.z = yaw*(msg.angular.z)

    if abs(msg.angular.z)<10*(np.pi/180):
        velocity.angular.z = 0
    ...
    # velocity.angular.z = 45*(np.pi/180)*np.sign(msg.angular.z)
    ...
    # # while(stopflag==True):
    # #     print("Stop")
    #     pub_commands.publish(Twist())
    #     rospy.sleep(3)
    #     stopflag = False

    # dt1 = rospy.Duration(secs=2)
    # if (rospy.Time.now() - time_init1) > dt1:
    #     stopflag = True
...
def window_callback(data):
    ...
    # if startflag == False:
    # error = data[0] - 320
    # velocity.angular.z = -1*yaw*error
    # print(velocity.angular.z)
...
def window():
    ...
    while not rospy.is_shutdown():
        ...
        # while(startflag!=True and stopflag!=True):
        #     print("Initial Vel")
...
def send_vel(velocity,flag):
    ...
    if flag == 1:
        ...
        if velocity==Twist():
            i = i + 1
            if i > 5:
                velocity.linear.z = velocity_prev.linear.z
                i = 0

    # if rospy.is_shutdown():
    #     break
    time_init = rospy.Time.now()
    dt1 = rospy.Duration(secs=1)
    while( (rospy.Time.now() - time_init)) < dt1:
        pub_commands.publish(velocity)
        # time.sleep(1)
    
    last_command_t=rospy.Time.now()
