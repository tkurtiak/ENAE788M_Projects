...
out = cv2.VideoWriter('Project3Threshold.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (320,240))
...
class image_converter:
	def __init__(self):
		...
		self.image_pub = rospy.Publisher("reducedImg",Image,queue_size=5)
		self.image_pubSG = rospy.Publisher("mask_out_SG",Image,queue_size=5)
		self.image_pubGMM = rospy.Publisher("mask_out_GMM",Image,queue_size=5)
	...
	def callback(self,data,args):
		...
		try:
			self.image_pub.publish(self.bridge.cv2_to_imgmsg(cv_image_small, "bgr8"))
			self.image_pubSG.publish(self.bridge.cv2_to_imgmsg(img_thresh_Single, encoding="passthrough"))
			self.image_pubGMM.publish(self.bridge.cv2_to_imgmsg(img_thresh_GMM, encoding="passthrough"))
		except CvBridgeError as e:
			print(e)
