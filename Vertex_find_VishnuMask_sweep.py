...
for subdir, dirs, files in os.walk(dirpath + '/Color Thresholding/Images/bag1_corrected'):
	files.sort()
	for file in files:
		...
		if filepath.endswith(".jpg") or filepath.endswith(".pgm") or filepath.endswith(".png") or filepath.endswith(".ppm"):
			...
			cv2.circle(img_fin,(int(center[0]),int(center[1])),3,(0,0,255),-1)
			for i in range(inner_corners.shape[0]):
				...
				cv2.circle(img_fin,(int(inner_corners[i,0]),int(inner_corners[i,1])),3,(255,0,0),-1)
				cv2.circle(img_fin,(int(outer_corners[i,0]),int(outer_corners[i,1])),3,(0,255,0),-1)
